...
def log_trade(analyst, channel_id, ticker, action,
             strike=None, expiry=None, price=None,
             status="Open", raw_text=None, message_id=None):
    sb = get_client()
    data = {
        "analyst": analyst,
...
def update_trade_status(trade_id, status, exit_price=None):
    sb = get_client()
...
def get_todays_closed_trimmed(today):
    sb = get_client()
    res = (
        sb.table("trades")
        .select("*")
