...
def _draw_analyst_card(draw, y, key, analyst_data):
    ...
    nf = _font(14, bold=True)
    draw.text((PAD + CARD_PAD, y + 10), name, font=nf, fill=LIGHT)

    if not trades:
        rf = _font(12)
        draw.text((PAD + CARD_PAD, y + ANALYST_H), 'No trades', font=rf, fill=MUTED)
    else:
...
def build_graphic(data: dict, trade_of_day: dict, date_str: str) -> bytes:
    ...
    if trade_of_day:
        ...
        draw.text((PAD + CARD_PAD, y + 24), ticker, font=tkf, fill=GOLD)
        if pnl:
            pf = _font(16, bold=True)
            pc = GREEN if pnl.startswith('+') else RED
            pw = draw.textlength(pnl, font=pf)
            draw.text((W - PAD - CARD_PAD - pw, y + 28), pnl, font=pf, fill=pc)
        if analyst:
            af = _font(11)
            draw.text((PAD + CARD_PAD, y + 56), analyst, font=af, fill=MUTED)
    y += HERO_H + PAD
