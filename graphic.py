...
def build_graphic(trades: list, date: str) -> bytes:
    ...
    img_h = HEADER_H + PADDING + rows * ROW_H + PADDING * 3
    ...
    font_row = _load_font(17)
    font_sm = _load_font(13)

    draw.text((PADDING, 14), "FiFi's TQE - Daily Trade Recap", font=font_title, fill=GOLD)
    draw.text((PADDING, 52), date + '  |  Trimmed & Closed Only', font=font_sub, fill=TEXT_COLOR)

    closed_count = len(closed_trades)
    trimmed_count = len(trimmed_trades)
    summary = f'{closed_count} Closed  |  {trimmed_count} Trimmed'
    draw.text((CARD_W - 220, 30), summary, font=font_sub, fill=ACCENT)
    ...
    if not shown:
        ...
        for i, trade in enumerate(shown):
            ...
            label = f'[{status}]'
            detail = f'{ticker}  {strike}  {expiry}'
            price_display = f'@ {price_s}' if price_s else ''
            ...
            draw.text((PADDING + 110, y + 30), price_display, font=font_sm, fill=ACCENT)
            draw.text((CARD_W - 170, y + 8), by_line, font=font_sm, fill=ACCENT)
            y += ROW_H
