...
async def eod_scheduler(bot):
    ...
    while True:
                ...
                if now_et >= fire_et:
                                tomorrow = now_et.date().toordinal() + 1
                                from datetime import date as _date
                                fire_et = fire_et.replace(day=_date.fromordinal(tomorrow).day,
                                                          month=_date.fromordinal(tomorrow).month,
                                                          year=_date.fromordinal(tomorrow).year)
